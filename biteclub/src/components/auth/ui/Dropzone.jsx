...
export function Dropzone() {
  ...
  const thumbs = files.map(file =>
    file.type.startsWith('image/') ? (
      ...
      <div
        key={file.name}
        className="flex items-center gap-2 px-2 py-1 text-sm text-gray-700 bg-gray-100 rounded border border-grey-300 shadow-sm"
      >
        <img width="16" height="16" src="https://img.icons8.com/material-rounded/24/upload--v1.png" alt="upload icon" />
  ...
  return (
    <section className="w-full max-w-xl mx-auto mt-2">
      ...
      >
        ...
        <p className="text-xs mt-1">(Accepts: PNG, JPG, JPEG, PDF | Max size: 10MB)</p>
      </div>

      {files.length > 0 && <aside className="mt-4 flex flex-wrap gap-3">{thumbs}</aside>}
    </section>
  );
}
