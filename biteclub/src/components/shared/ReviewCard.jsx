    ...
    >
      <div className="p-4">
        <div className="flex justify-between">
          ...
          <div>
            <EngagementIconStat iconArr={reviewCardIconArr} statNumArr={[review.likes.count, review.comments.length]} />
          </div>
        </div>
        <h3>{review.title}</h3>
        ...
        <p>{review.body}</p>
        <AuthorDateBlurb authorPic="/img/profilepic.jpg" authorName="AUTHOR NOT DYNAMIC" date={review.date_posted} />
      </div>

      {photos?.length > 0 && (
        <div className="flex gap-x-1">
          <div className="relative w-1/2 h-50 overflow-hidden">
            <Image src={photos[0].url} alt={photos[0].caption} className="object-cover rounded-bl-md " fill={true} />
          </div>
          {photos[1] && (
            <div className="relative w-1/2 h-50 overflow-hidden">
